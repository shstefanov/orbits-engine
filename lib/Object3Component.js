...
const materialSides = { FrontSide, BackSide, DoubleSide };
...
class Object3Component extends Component {
	...
	handleMaterial(material){
		if(this.material && typeof material === "object"){
			const { emissive, color, side, ...rest} = material;
			Object.assign(this.material, rest);
			emissive && this.material.emissive.setHex( parseInt(emissive.replace("#", "0x"), 16) );
			color    && this.material.color.setHex( parseInt(color.replace("#", "0x"), 16) );
			side     && (this.material.side = materialSides[side] || FrontSide);
		}
	}
	...
	handleColor(color){
		if(typeof color === "string" && this.material){
	...
	handleTransition(transition){
		...
		const base = {
			...
			color:    (typeof from.color === "string") ? deconstructColor(from.color) : false,
			opacity:  (typeof from.opacity === "number") ? from.opacity : false,
		}
		...
		const diff = {
			...
			color:    (base.color !== false) ? colorDiff(base.color, deconstructColor(to.color)) : false,
			opacity:  (base.opacity !== false) ? to.opacity - from.opacity : false,
		}
		...
		this.transition({
			...
			const result = {
				...
				color:    (base.color !== false) ? colorAdd(base.color, multiplyColor(diff.color, path)) : false,
				opacity:  (base.opacity !== false) ? ( base.opacity + ( diff.opacity * path) ) : false,
			};
			this.handlePosition(result.position);
			this.handleRotation(result.rotation);
			this.handleScale(result.scale);
			(base.color !== false) && this.handleColor(stringifyColor(result.color));
			(base.opacity !== false) && this.handleOpacity(result.opacity);
		});
	...
	handlePeriod(period_settings){
		...
		this.period(period, (t, path) => {
			...
			const result = {
				position: from.position && vecAdd(base.position, multiplyScalar(diff.position, value)),
				rotation: from.rotation && vecAdd(base.rotation, multiplyScalar(diff.rotation, value)),
				scale:    from.scale    && vecAdd(base.scale,    multiplyScalar(diff.scale,    value)),
				color:    (base.color !== false) ? colorAdd(base.color,  multiplyColor(diff.color, path)) : false,
				opacity:  (base.opacity !== false) ? ( base.opacity + ( diff.opacity * path) ) : false,
			};
			from.position && this.handlePosition(result.position);
			from.rotation && this.handleRotation(result.rotation);
			from.scale    && this.handleScale(result.scale);
			(base.color !== false) && this.handleColor(stringifyColor(result.color));
			(base.opacity !== false) && this.handleOpacity(result.opacity);
		});
		this.__period = true;
	...
	handleFreezeControls(freezeControls){
		console.log("handleFreezeControls: ", !freezeControls);
