...
class Object3Component extends Component {
	...
	async loadElement(geometry, material){
		...
		if(material){
			...
			else if(material.type === "WireframeMaterial"){
				this.material = new THREE.WireframeMaterial(this.geometry);
			}
			else {
	...
	createMaterial({ type, textures, ...props}){
		...
		this.material = new Proto(convertMaterialProps(props, textures));
	}
