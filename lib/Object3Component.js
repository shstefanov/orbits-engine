...
class Object3Component extends Component {
	...
	async loadMaterial(){
		...
		// if(this.context.materialsCache.has(src)){
		// 	this.material = this.context.materialsCache.get(src).clone();
		// 	return;
		// }
		this.material = await ( new Promise( (resolve, reject) => {
	 	...
	 	// this.context.materialsCache.set(src, this.material.clone());
	}


	async loadGeometry(){
		const src = this.props.geometry;
		if(this.context.geometriesCache.has(src)){
			this.geometry = this.context.geometriesCache.get(src).clone();
			return;
		}
	...
	UNSAFE_componentWillReceiveProps(nextProps){
		...
		this.handleFreezeControls(nextProps.freezeControls);
	}
