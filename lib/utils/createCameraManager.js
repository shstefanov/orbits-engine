...
export default function createCameraManager(camera, props, renderer, initialized){
    ...
    if(props.controlType === "orbit-controls"){
        ...
        // onUpdate listener
        if(!camera) effects.unshift(uePlaceholder0);
        else effects.unshift( ({ onUpdate, ...props }, apply) => apply(() => {
            let action = null;

            function rotateHandler(event, isTouch){
                ...
                const endListener = e => {
                    ...
                    window.removeEventListener("touchleave", touchLeaveListener);
                    window.removeEventListener("touchmove",  touchMoveListener);
                };
                
                let touchMoveListener, touchLeaveListener, touchEndListener;
                window.addEventListener("mouseup",    endListener);
                window.addEventListener("mouseleave", endListener);
                window.addEventListener("mousemove",  moveListener);

                window.addEventListener("touchend",   touchEndListener   = e => endListener());
                window.addEventListener("touchleave", touchLeaveListener = e => endListener());
                window.addEventListener("touchmove",  touchMoveListener  = e => moveListener(e.touches[0]));
            ...
            canvas.addEventListener("mousewheel",  scrollHandler,      { passive: true } );
            ...
            return () => {
                ...
                touchListener && canvas.removeEventListener("touchstart", touchStartListener );
            };
        }, []) );
