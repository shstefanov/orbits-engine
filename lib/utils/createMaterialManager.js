...
export default function createMaterialManager(material, props, renderer, initialized){
    ...
    for(let propName in props) {
        ...
        else if(propName === "uniforms")         effects.push( createUniformsManager         ( material, propName, renderer, props ) );
...
function uePlaceHolderN(n){ return () => useEffect(v, Array(n)); }
...
function createUniformsManager ( material, propName, renderer, _props ) {
    return !material ? uePlaceHolderN( Object.keys(_props[propName]).length ) : (props) => useEffect( () => {
        const newUniforms = props[propName];
        for(let key in newUniforms){
            if(!material.uniforms[key]) material.uniforms[key] = {};
            material.uniforms[key].value = newUniforms[key];
        }
        material.uniformsNeedUpdate = true;
        renderer.render();
    }, Object.values(props[propName]));
}
...
function createTextureManager(material, propName, renderer){
    ...
    return !material ? uePlaceholder1 : (props) => useEffect( () => {
        ...
        if(typeof props[propName] === "string"){ // It is url
            const texture = material[propName] = new THREE.TextureLoader().load( props[propName] );
            material.needsUpdate = true;
            renderer.waitFor( () => texture.source.data && renderer.render() );
        }
        else { // It is instance of texture
            const texture = material[propName] = props[propName];
            renderer.waitFor( () => texture.source.data && renderer.render() );
        }
