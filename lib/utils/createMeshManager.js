...
export default function createMeshManager(mesh, props, renderer, initialized, effects = []){
    ...
    // This is in case we are extending effects set with this manager
    ...
    if(props.hasOwnProperty("interactive")){
        ...
        if(mesh) {
            ...
            mesh.setTitle  = renderer.setTitle;
            ...
            else if(props.hasOwnProperty("cursor") || props.hasOwnProperty("title")){
    ...
    props.hasOwnProperty("overlay") && effects.push(
        !mesh ? uePlaceholder1 : (({ overlay, renderOrder = 0 }, apply) => apply( () => {
            ...
            if(overlay){
                render = true;
                mesh.noOverlayProps = mesh.noUnderlayProps || {
    ...
    // Handle 'underlay' attribute if present
    props.hasOwnProperty("underlay") && effects.push(
        !mesh ? uePlaceholder2 : (({ overlay, underlay, renderOrder = 0 }, apply) => apply( () => {
            if(overlay) return; // Overlay is with priority
            let render = false;
            if(underlay){
                render = true;
                mesh.noUnderlayProps = mesh.noOverlayProps || {
                   depthFunc:  mesh.material.depthFunc,
                   depthTest:  mesh.material.depthTest,
                   depthWrite: mesh.material.depthWrite,
                };
                ...
                mesh.material.depthFunc = THREE.NeverDepth;
            }
            else if(mesh.noUnderlayProps) {
                render = true;
                mesh.renderOrder = renderOrder;
                mesh.material.depthFunc  = mesh.noUnderlayProps.depthFunc;
                mesh.material.depthTest  = mesh.noUnderlayProps.depthTest;
                mesh.material.depthWrite = mesh.noUnderlayProps.depthWrite;
                delete mesh.noUnderlayProps.depthFunc;
            }
            if(render){
                mesh.material.needsUpdate = true;
                renderer.render();
    ...
    // Only for THREE.Sprite
    props.hasOwnProperty("center") && effects.push(
        !mesh ? uePlaceholder2 : (({ center: {x, y} }, apply) => apply( () => {
            mesh.center.set(x, y);
            renderer.render();
        }, [x, y]))
    );
    ...
    props.hasOwnProperty("cursor") && props.hasOwnProperty("interactive") && effects.push(
        !mesh ? uePlaceholder2 : (({cursor = "default", interactive}, apply) => apply( () => {
            interactive  && mesh.setCursor(cursor);
            !interactive && mesh.setCursor("default");
        }, [cursor, interactive]))
    );

    // Handle Title Change
    props.hasOwnProperty("title") && props.hasOwnProperty("interactive") && effects.push(
        !mesh ? uePlaceholder2 : (({title = "", interactive}, apply) => apply( () => {
            interactive && mesh.setTitle(title);
            !interactive && mesh.setTitle("");
        }, [title, interactive]))
    );
    ...
    // Only if mesh is line
    props.hasOwnProperty("points") && props.hasOwnProperty("_computeLineDistances") && effects.push(
    ...
    // instanceMatrix and instanceCount are produced by intermediate component
    props.hasOwnProperty("instanceMatrix") && effects.push(
        !mesh ? uePlaceholder4 : ({ instanceMatrix, instanceCount, instances }, apply) => apply(() => {
            mesh.instanceMatrix             = instanceMatrix;
            mesh.instanceMatrix.needsUpdate = true;
            mesh.count                      = instanceCount;
            renderer.render();
        }, [ instanceMatrix, instanceMatrix.state, instanceCount, instances?.state ])
    );
...
function handleHoverInElement(){
    ...
    this.userData.hasOwnProperty("title")  && this.setTitle(this.userData.title);
}
function handleHoverOutElement(){
    this.setShowHoverElement(false);
    this.userData.hasOwnProperty("cursor") && this.setCursor(null);
    this.userData.hasOwnProperty("title")  && this.setTitle(null);
}
...
function toggleMaterialTransition(renderer, direction){
    ...
    if( this.userData.hasOwnProperty("title")) {
        this.setTitle(direction === "in" ? this.userData.title : null);
    }
