...
export default function createMeshManager(mesh, props, renderer, initialized, effects = []){
    ...
    props.hasOwnProperty("instanceMatrix") && effects.push(
        !mesh ? uePlaceholder3 : ({ instanceMatrix, instanceCount, instances }, apply) => apply(() => {
            console.log("meshManager: instanceMatrix", instanceMatrix.array);
            mesh.instanceMatrix             = instanceMatrix;
            mesh.instanceMatrix.needsUpdate = true;
            mesh.count                      = instanceCount;
            renderer.render();
        }, [ instanceMatrix, instanceCount, instances?.state ])
    );
