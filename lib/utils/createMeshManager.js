...
export default function createMeshManager(mesh, props, renderer, initialized, effects = []){
    ...
    if(props.hasOwnProperty("interactive")){
        ...
        if(mesh) mesh.setCursor = renderer.setCursor;

        // Mesh has hover
        if(props.hasOwnProperty("hover") || props.hasOwnProperty("cursor") ){
            ...
            effects.push(
                !mesh ? uePlaceholder1 : (({ interactive }, apply) => apply( () => {
                    ...
                    return () => {
                        if(interactive){console.error("TODO: Implement MouseLock camera control for orthographic camera");
                            renderer.removeMouseInteractiveObject(mesh);
                            renderer.render();
                        }
                    }
